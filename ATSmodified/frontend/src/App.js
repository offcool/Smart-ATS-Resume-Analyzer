...
function App() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/analyze', {  // Correct relative URL
                method: 'POST',
                body: formData,
            });
            ...
            setResult({
                'JD Match': 'Error',
                'MissingKeywords': [error.message || 'An error occurred during analysis. Please try again.'],
                'Profile Summary': 'Failed to retrieve analysis results.',
            });
            alert(`⚠️ Analysis failed. ${error.message || 'Please check the backend and try again.'}`);
        } finally {
            setLoading(false);
        }
    };
